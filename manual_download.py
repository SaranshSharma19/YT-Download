...
if __name__ == "__main__":
    ...
    print("\nProcessing segments:")
        ...
        # Prompt user for start and end times in MM:SS format
    start_time_input = input("Enter the start time in 'MM:SS' format (e.g., 5:30): ")
    end_time_input = input("Enter the end time in 'MM:SS' format (e.g., 6:00): ")
        
    try:
        start_time = convert_to_seconds(start_time_input)
        end_time = convert_to_seconds(end_time_input)
            
        if end_time - start_time >= 10:  # Ensure the segment is at least 10 seconds long
            minutes_start = int(start_time) // 60
            seconds_start = int(start_time) % 60
            minutes_end = int(end_time) // 60
            seconds_end = int(end_time) % 60
            start_time_formatted = f"{minutes_start}:{seconds_start:02d}"
            end_time_formatted = f"{minutes_end}:{seconds_end:02d}"
                
            print(f"Processing segment: {start_time_formatted} to {end_time_formatted}")
            downloader.process_segment(start_time, end_time, 1)  # Assuming part_num is 1
        else:
            print("The segment must be at least 10 seconds long.")
