...
class EnhancedTradingBot:
    ...
    def train_model(self, symbol):
        """Train model for a specific index with optimizations"""
        try:
            data = self.fetch_data(symbol)
            if data is None or len(data) < 100:  # Increased data requirement
            ...
            grid_search = GridSearchCV(rf, param_grid, cv=5, n_jobs=-1)  # Increased cv for better validation
            grid_search.fit(X, y)
